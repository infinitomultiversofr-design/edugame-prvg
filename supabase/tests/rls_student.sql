...
SELECT set_config('request.jwt.claim.sub', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaa01', true);
SELECT set_config(
  'request.jwt.claims',
  json_build_object('sub','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaa01','role','authenticated')::text,
  true
);
SET LOCAL ROLE authenticated;

SELECT extensions.is((SELECT count(*)::int FROM public.students), 1, 'student vê apenas o próprio estudante');
SELECT extensions.is((SELECT min(edugame_id) FROM public.students), 'M0-A-001'::text, 'student vê o próprio edugame_id');
SELECT extensions.is((SELECT count(*)::int FROM public.classes), 1, 'student vê apenas a própria turma');
SELECT extensions.is((SELECT count(*)::int FROM public.enrollments), 1, 'student vê apenas a própria matrícula');
