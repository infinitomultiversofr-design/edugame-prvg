...
SELECT set_config('request.jwt.claim.sub', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaa30', true);
SELECT set_config(
  'request.jwt.claims',
  json_build_object('sub','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaa30','role','authenticated')::text,
  true
);
SET LOCAL ROLE authenticated;

SELECT extensions.is((SELECT count(*)::int FROM public.students), 2, 'coordenação vê estudantes da escola A');
SELECT extensions.is((SELECT count(*)::int FROM public.schools), 1, 'coordenação vê apenas sua escola');
SELECT extensions.is((SELECT count(*)::int FROM public.classes), 2, 'coordenação vê classes da escola A');
SELECT extensions.is((SELECT count(*)::int FROM public.students WHERE school_id = '20000000-0000-0000-0000-000000000001'), 0, 'coordenação não vê escola B');
